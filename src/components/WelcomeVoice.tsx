...
export const WelcomeVoice = () => {
  ...
  useEffect(() => {
    const fetchProfileAndSpeak = async () => {
      ...
      // Verificar se já falou nesta sessão de login
      const sessionKey = `welcome_spoken_${user.id}`;
      const hasSpokenThisSession = sessionStorage.getItem(sessionKey);
      
      if (hasSpokenThisSession || hasSpoken) return;
      ...
      try {
        ...
        // Marcar como já falado nesta sessão
